...
function alertMsg(message){$('#alert').innerHTML=message?`<div class="error">${message}</div>`:''}
function showPage(id){document.querySelectorAll('.page').forEach(x=>x.classList.toggle('active',x.id===id));document.querySelectorAll('nav button').forEach(x=>x.classList.toggle('active',x.dataset.page===id));$('#pageTitle').textContent=pages.find(x=>x[0]===id)[1];if(id==='settings')loadSettings();if(id==='drive')loadDrive()}
function buildNav(){pages.forEach(([id,,label])=>{const b=document.createElement('button');b.dataset.page=id;b.innerHTML=`<span>${label}</span>`;b.onclick=()=>showPage(id);$('#nav').append(b)});$('#nav button').classList.add('active')}
